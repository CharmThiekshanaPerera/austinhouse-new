...
import { Trash2, Search, Download, UserPlus } from "lucide-react";
...
const AdminNewsletter = () => {
  const { newsletterSubscribers, addNewsletterSubscriber, updateNewsletterSubscriber, deleteNewsletterSubscriber } = useData();
  ...
  const handleAdd = () => {
    if (!newEmail || !newEmail.includes("@")) { toast({ title: "Error", description: "Enter a valid email.", variant: "destructive" }); return; }
    addNewsletterSubscriber(newEmail);
    setNewEmail("");
          ...
          <Card key={s.id} className="border-border">
            <CardContent className="p-4 flex items-center gap-4">
              <div className="flex-1 min-w-0">
                ...
                <p className="text-muted-foreground font-body text-xs">Subscribed: {format(new Date(s.subscribedAt), "MMM d, yyyy")}</p>
              </div>
              <div className="flex items-center gap-3 flex-shrink-0">
                <div className="flex items-center gap-2">
                  <Switch checked={s.active} onCheckedChange={v => updateNewsletterSubscriber({ ...s, active: v })} />
                ...
                <Button size="sm" variant="destructive" onClick={() => { deleteNewsletterSubscriber(s.id); toast({ title: "Removed" }); }}>
                  <Trash2 size={14} />
                </Button>
              </div>
            </CardContent>
